...
struct ShaderAssertsVisitor
{
	...
	void InlineShaderIncludesInternal(const std::filesystem::path& outFolder, const RenderGraph& renderGraph, std::string& shaderFileString, std::string& result, std::unordered_set<std::string>& resolvedFiles)
	{
		size_t offset = 0;
		...
		for (const auto& includedFile : includedFiles)
		{
            ...
            std::string actualFilename = includedFile.filename;
            for (const auto& fileCopy : renderGraph.fileCopies)
            {
                if (fileCopy.destFileName == includedFile.filename)
                {
                    actualFilename = fileCopy.fileName;
                }
            }

			std::string srcFileName = (outFolder / actualFilename).string();
			...
			InlineShaderIncludesInternal(outFolder, renderGraph, includedShaderFile, result, resolvedFiles);
	...
	void InlineShaderIncludes(std::vector<unsigned char>& shaderFile, const char* outFolder, const RenderGraph& renderGraph)
	{
		std::string result;
		std::unordered_set<std::string> resolvedFiles;

		std::filesystem::path copyOutFolder = std::filesystem::path(outFolder);

		std::string shaderFileString = (char*)shaderFile.data();
		InlineShaderIncludesInternal(copyOutFolder, renderGraph, shaderFileString, result, resolvedFiles);
    ...
    ShaderParsingResult ParseShader(Shader& shader, const std::string& path)
    {
        ...
        InlineShaderIncludes(fileContents, renderGraph.baseDirectory.c_str(), renderGraph);
